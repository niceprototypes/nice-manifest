...
**Publish step behaviour (2026-09-26, toolkit `56adab8`).** `nicely publish` runs each `npm publish` asynchronously with a per-package timeout (`--publish-timeout <s>`, default 120). Any npm line containing a URL or a prompt is printed live above the spinner; routine `npm notice` output stays hidden. A timeout marks that package failed and the run continues. With npm 11 (`auth-type=web`), a rejected or expired OTP fails immediately with `EOTP` because stdout is not a TTY, and nicely re-prompts for a fresh code (up to 3×). `package.json` swaps are restored on success, failure, Ctrl-C, SIGTERM and terminal close (SIGHUP) — previously a closed terminal or Ctrl-C left the swaps on disk.
