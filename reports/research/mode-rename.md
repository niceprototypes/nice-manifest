...
1. **CSS variable suffix stays as `--day` / `--night`.** The rename targets the API layer, not the CSS authoring layer. Variables like `--np--color--base--day` and `--np--color--base--night` keep their existing names; no consumer's hand-written CSS breaks.
