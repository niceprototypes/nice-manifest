...
**Exports:**
- Token getters: `getToken()` / `getTokenKey()` / `getTokenValue()` (registry-aware, runtime-extensible via `createTokens`); convenience wrappers `getColorToken*`, `getSizeToken*`, `getComponentToken*` with the same three-form pattern.
...
- Setters: `setCoreTokens`, `setColorTokens`, `setSizeTokens`, `setBreakpoints`, `registerTokens`.
...
- `src/generated/` — auto-generated by build scripts (tokensData.ts, colorTokensData.ts, sizeTokensData.ts, componentTokensData.ts, breakpointsData.ts, types.ts).
