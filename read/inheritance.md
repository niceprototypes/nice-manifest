...
**Exports:**
- Functions: `getReactToken()`, `getConstant()`, `getComponentToken()`, `getBreakpoint()`
...
**Internal structure:**
- `src/services/` — public API (getReactToken, getConstant, getComponentToken, getBreakpoint)
...
- `src/tokens/` — source JSON: `core/{default,night}/`, `component/{button,icon,lightbox,tile,typography}/`
...
- `--clean-all`: Recursively clean singletons (react, styled-components, etc.) from linked packages' `node_modules`
...
- Function: `getReactToken()` - unified token accessor (registry-based)
- Functions: `registerTokens()`, `hasToken()`, `getReactTokenNames()`, `getReactTokenVariants()`, `getReactTokenModes()`, `getReactTokenModeValue()`, `isModeValue()` - registry utilities
- Constants: `DEFAULT_MODE` (`"day"`)
- Re-exports entire nice-styles public API: all functions (`getBreakpoint`, `getToken`, `getColorToken`, `getSizeToken`, `setCoreTokens`, `setColorTokens`, `setSizeTokens`, `getConstant`, `NAMESPACE`, `getComponentToken`, `getInvertedMode`, `getTextHeight`, `getTokenFromMap`, `getTokenByPath`, `camelToKebab`, `Theme`, `componentTokensData`), all types, and all token union types
