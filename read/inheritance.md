...
**Exports:**
- Token getters: `getToken()` / `getTokenKey()` / `getTokenValue()` (registry-aware, runtime-extensible via `createTokens`); convenience wrappers `getModeToken*`, `getBreakpointToken*`, `getComponentToken*` with the same three-form pattern.
...
- Setters: `setCoreTokens`, `setModeTokens`, `setBreakpointTokens`, `setBreakpoints`, `registerTokens`.
...
- Style-value helpers: `isStyleValue()`, plus types `ModeValue`, `BreakpointValue`, `StyleValueKind`.
- Constants: `NAMESPACE` (`"np"`), `DEFAULT_MODE`, `DEFAULT_BREAKPOINT`, `STYLE_VALUE_KEYS`, `BREAKPOINT_PHONE/TABLET/LAPTOP/DESKTOP`, `BREAKPOINTS`.
...
**Internal structure:**
- `src/services/` — public functions (getToken family, getConstant, getComponentToken, getBreakpoint, set*Tokens, generateTokenCSS, parseGoogleFontsUrl).
- `src/registry/` — runtime token registry singleton + seeding (createRegistry, registerTokens, seedDimensionedTokens). Seed runs at module load via `init.ts`.
- `src/utilities/` — internal (camelToKebab, camelToScreaming, formatError, getTokenFromMap, isStyleValue, tokenStyleSheet, breakpointStyleSheet, buildSizeMediaCss).
- `src/types/` — style-value and Google Fonts type modules.
- `src/constants/` — breakpoints, styleValues (DEFAULT_MODE/BREAKPOINT, STYLE_VALUE_KEYS).
- `src/generated/` — auto-generated by build scripts (tokensData.ts, modeTokensData.ts, breakpointTokensData.ts, componentTokensData.ts, breakpointsData.ts, types.ts).
- `src/tokens/` — source JSON (module.json, module.modes.json, module.breakpoints.json, component.json, breakpoints.json).
...
│       └── index.ts                 # Re-exports
├── tokens/
│   ├── {Component}Styles.ts         # No-op component (CSS in nice-styles tokens.css)
